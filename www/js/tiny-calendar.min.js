...
angular.module('angular-tiny-calendar', []).directive('tinyCalendar', [function() {
    ...
    return {
        ...
        template: function(element, attrs) {
            ...
            html += "<div ng-if='userType==0' ng-repeat='day in month' class='tc-day' ng-class='{ differentMonth: !day.isCurrentMonth, selected: day.date === selected.date, statusPink: day.events.length > 0}' ng-click='select(day)'>{{day.number}}</div>";
            html += "<div ng-if='userType==1' ng-repeat='day in month' class='tc-day' ng-class='{ differentMonth: !day.isCurrentMonth, selected: day.date === selected.date, statusBlue: day.events.length > 0 && day.events[0].status==0, statusGreen: day.events.length > 0 && day.events[0].status==1}' ng-click='select(day)'>{{day.number}}</div>";
            ...
            html += "<ion-item ng-if='userType==1' ng-repeat='e in events | orderBy:&quot;date&quot;' class='item item-icon-left'><i ng-class='{statusBlue: e.status==0,statusGreen: e.status==1}'>{{e.startDate | date: 'dd'}}</i>{{e.status==1 ? 'Status: Proposta Aprovada':'Status: Oferta Feita'}}</ion-item>";
